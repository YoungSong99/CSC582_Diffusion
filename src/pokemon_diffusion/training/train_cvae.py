...
def train_one_epoch(model, dataloader, optimizer, recon_loss_fn, device, beta):
    ...
    for batch in tqdm(dataloader, desc="Training", leave=False):
        ...
        color_id = batch["color_id"].to(device)
        shape_id = batch["shape_id"].to(device)

        recon, mu, logvar = model(x, type_id, color_id, shape_id)
...
def train_cvae_model(model, train_loader, val_loader, params):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)

    experiment_name = params.get("experiment_name", "cvae_experiment")
